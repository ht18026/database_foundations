...
COMMENT ON COLUMN trip.lang_iso_code IS 
    'ISO 639-1 two-character language codes';
...
ALTER TABLE trip
    ADD CONSTRAINT location_pickup_trip_fk FOREIGN KEY (pickup_locn_id)
        REFERENCES location (locn_id);

ALTER TABLE trip
    ADD CONSTRAINT location_dropoff_trip_fk FOREIGN KEY (dropoff_locn_id)
        REFERENCES location (locn_id);
